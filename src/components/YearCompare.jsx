...
export default function YearCompare({ compare }) {
  ...
  return (
    <div className="space-y-5 md:space-y-2.5">
      <p className="text-sm leading-7 text-cyan-50/70 md:text-[0.68rem] md:leading-4">
        ...
        <ul className="space-y-3.5 md:space-y-2">
          {(compareData?.years || []).map((yearRow) => {
            ...
            return (
              <li
                key={yearRow.year}
                className="rounded-[1.4rem] border border-cyan-100/10 bg-[linear-gradient(180deg,rgba(9,31,48,0.72),rgba(3,15,24,0.9))] px-5 py-5 md:rounded-[0.9rem] md:px-2.5 md:py-2.5"
              >
                <div className="flex flex-wrap items-start justify-between gap-x-3 gap-y-2.5 md:gap-y-1">
                  <div className="min-w-0">
                    <p className="text-xl font-semibold tracking-[-0.03em] text-white tabular-nums md:text-sm">
                      {yearRow.year}
                    </p>
                    {windowLabel && (
                      <p className="mt-1.5 text-sm text-cyan-100/50 md:mt-0.5 md:text-[0.68rem]">
                        {windowLabel}
                      </p>
                    )}
                  </div>
                  ...
                  <span
                    className={`inline-flex shrink-0 items-center gap-2 rounded-full border border-cyan-100/10 bg-white/5 px-3.5 py-2 text-sm font-medium md:gap-1 md:px-2 md:py-0.5 md:text-[0.68rem] ${meta.textClass}`}
                  >
                    <span
                ...
                {yearRow.status !== "no_surveys" && (
                  <div className="mt-4 grid grid-cols-3 gap-2.5 md:mt-2 md:gap-1.5">
                    {[
                      ...
                      <div
                        key={item.label}
                        className="rounded-[1rem] border border-cyan-100/10 bg-[#03131f]/70 px-3 py-3 md:rounded-[0.75rem] md:px-2 md:py-1.5"
                      >
                        <p className="creel-label text-cyan-100/50 md:text-[0.62rem]">
                          {item.label}
                        </p>
                        <p className="mt-1.5 text-lg font-semibold tracking-[-0.03em] text-white tabular-nums md:mt-0.5 md:text-sm">
                ...
                {speciesEntries.length > 0 && (
                  <div className="mt-4 flex flex-wrap gap-2 md:mt-2 md:gap-1">
